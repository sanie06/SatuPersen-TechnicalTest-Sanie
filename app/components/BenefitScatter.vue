...
<template>
  <div
    ref="el"
    class="grid gap-4 sm:grid-cols-2 lg:relative lg:mx-auto lg:block lg:h-[19rem] lg:max-w-4xl"
  >
    <div
    ...
    >
      ...
      <div
        ...
        class="flex h-full flex-col gap-3 rounded-3xl border border-brand-200/70 bg-[linear-gradient(to_top_left,rgba(13,13,26,0.4),transparent_55%),linear-gradient(to_bottom_right,#fce047,#f5c518)] p-6 text-ink-950 shadow-2xl shadow-brand-500/20 transition duration-300 hover:-translate-y-1 hover:border-white hover:shadow-brand-500/40 lg:pr-16"
      >
        <div class="flex h-11 w-11 items-center justify-center rounded-2xl bg-ink-950/10">
          <UIcon :name="benefit.icon" class="h-5 w-5 text-ink-950" />
        </div>
        <h3 class="font-bold">{{ benefit.title }}</h3>
        <p class="text-sm leading-relaxed text-ink-950">{{ benefit.description }}</p>
      </div>
    </div>
  </div>
</template>
